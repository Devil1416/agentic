...
def _quick_classify(text: str) -> dict | None:
    ...
    # Explicit build/create/run commands
    build_patterns = [
        r'^(build|create|make|generate|write|implement|code)\b',
        r'^(set up|setup|scaffold|bootstrap)\b',
        r'^(run)\b'
    ]
    for pat in build_patterns:
        if re.match(pat, lower):
            return {
                "mode": "EXECUTE",
                "goal": text,
                "response": f"On it. Let me execute that for you.",
            }
